...
{
    const u32 *image;
    const u16 *palette; // u32 previously
    const u32 *tilemap;
};
...
void HnS_LaunchBattleAnimation(const u8 *const animsTable[], u16 tableId, bool8 isMoveAnim);
...
#define STAT_ANIM_PLUS1  (MOVE_EFFECT_ATK_PLUS_1 - 1)  // 14 previously
#define STAT_ANIM_PLUS2  (MOVE_EFFECT_ATK_PLUS_2 - 1)  // 38 previously
#define STAT_ANIM_MINUS1 (MOVE_EFFECT_ATK_MINUS_1 - 1) // 21 previously
#define STAT_ANIM_MINUS2 (MOVE_EFFECT_ATK_MINUS_2 - 1) // 45 previously
