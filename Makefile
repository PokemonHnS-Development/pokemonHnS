...
MODERN_TESTELF = $(MODERN_ROM_NAME:.gba=-test.elf)
MODERN_HEADLESSELF = $(MODERN_ROM_NAME:.gba=-test-headless.elf)
...
ifeq ($(MODERN),0)
  ...
  ROM := $(MODERN_ROM_NAME)
  ifeq ($(MODERN_TESTELF),$(MAKECMDGOALS))
    TEST := 1
  endif
  ifeq ($(TEST), 0)
...
CPPFLAGS := $(INCLUDE_CPP_ARGS) -Wno-trigraphs -DMODERN=$(MODERN) -DTESTING=$(TEST) -std=gnu17
...
ifeq ($(MODERN),0)
  ...
  override CFLAGS += -mthumb -mthumb-interwork -O$(O_LEVEL) -mabi=apcs-gnu -mtune=arm7tdmi -march=armv4t -Wno-pointer-to-int-cast -std=gnu17 -Werror -Wall -Wno-strict-aliasing -Wno-attribute-alias -Woverride-init # -fno-toplevel-reorder 
  ifeq ($(ANALYZE),1)
...
RULES_NO_SCAN += libagbsyscall clean clean-assets tidy tidymodern tidynonmodern tidycheck generated clean-generated
...
tidy: tidymodern tidynonmodern tidycheck tidydebug
